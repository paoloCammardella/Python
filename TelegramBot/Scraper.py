...
def check_price(URL, update):
    ...
    if(soup.find(id = "priceblock_ourprice")):
        if(soup.find(id = "priceBadging_feature_div")):
            price = soup.find(id = "priceblock_ourprice").get_text()
            converted_price = float(price[0:5])

            if(converted_price < 3.000):
                send_telegramMessage(URL, title, price, update)
    else:
        print("")
        

def send_telegramMessage(URL, title, price, update):
    updater = Updater(keys.APYKey, use_context = True)
    update.message.reply_text("Sasi merda per questa \n" + title.strip() + " " + URL + "\n" + price.strip())